...
<body>
    ...
    try {
        ...
        $stmt = $conn->query("SELECT * FROM josaa.final_data  WHERE Institute='$institute' AND Gender='$gender' AND Round='$round' AND Seat_Type='$seat'");

        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            if ($course == "ALL" || (strpos($row['Academic_Program_Name'], $course) !== false)) {
                $years[] = $row['Year'];
        ...
        foreach ($unique_branches as $branch) {
            ...
            $stmt->execute();
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                if ($row['Academic_Program_Name'] == $branch) {
                    $yrs[] = $row['Year'];
                    array_push($opr, $row['Opening_Rank']);
                }
            }
            array_shift($yrs);
            array_shift($opr);
